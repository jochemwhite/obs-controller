...
interface RefreshTwitchTokenResponse {
  access_token: string;
...
type TwitchIntegration =
  Database["public"]["Tables"]["integrations_twitch"]["Row"];
...
export async function getChannelRefreshToken(
  channelId: string,
): Promise<string> {
  const { data, error } = await supabase
    .from("integrations_twitch")
...
export async function getTwitchAppToken(): Promise<{
  access_token: string;
  expires_in: number;
  updated_at: string;
}> {
  const { data, error } = await supabase
    ...
    .select(
      "access_token_ciphertext, access_token_iv, access_token_tag, expires_in, updated_at",
    )
    .single();
...
export async function updateTwitchAppToken(
  accessToken: string,
  expiresIn: number,
): Promise<void> {
