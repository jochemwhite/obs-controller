...
export const TwitchActionHandlers: Record<
  string,
  (event: ActionEvent, twitchApi: TwitchApi) => Promise<any>
> = {
  create_marker: async (event, twitchApi) => {
    const description: string = event.currentActionContext.description;

    const resolvedDescription = await resolveVariables(
      description,
      { twitchApi },
      event.results,
    );
  ...
  send_shoutout: async (event, twitchApi) => {
    const to_broadcaster_id = await resolveVariables(
      event.currentActionContext.to_broadcaster_id,
      { twitchApi },
      event.results,
    );
  ...
  send_announcement: async (event, twitchApi) => {
    const message = await resolveVariables(
      event.currentActionContext.message,
      { twitchApi },
      event.results,
    );
  ...
  create_clip: async (event, twitchApi) => {
    ...
    if (!clipData) throw new Error("Failed to create clip");
    const clipId = clipData.edit_url.split("/")[3];

    const broadcaster_name = await resolveVariables(
      event.currentActionContext.broadcaster_login,
      { twitchApi },
      event.results,
    );
