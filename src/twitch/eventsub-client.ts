...
export class TwitchEventSubReceiver {
  ...
  private async reconnect(): Promise<void> {
    if (
      ...
      console.log(
        "⏳ Already attempting to connect, skipping duplicate reconnect",
      );
      return;
    }
  ...
  private async handleMessage(message: WebSocketMessage): Promise<void> {
    ...
    switch (metadata.message_type) {
      case "session_welcome":
        this.sessionId =
          (payload as { session?: { id?: string } }).session?.id || null;
        this.reconnectUrl =
          (payload as { session?: { reconnect_url?: string } }).session
            ?.reconnect_url || null;
        this.lastKeepaliveTime = Date.now();

        const welcomeSession = (
          payload as { session?: { keepalive_timeout_seconds?: number } }
        ).session;
        ...
        if (this.conduitId && this.sessionId) {
          try {
            console.log(
              "Updating conduit shards with session ID:",
              this.sessionId,
            );
            await this.twitchApi.eventsub.updateShardTransport(
              this.conduitId,
              "0",
              {
                method: "websocket",
      ...
      case "session_reconnect":
        ...
        if (this.reconnectUrl) {
          ...
          console.error(
            "❌ Reconnect URL not provided in session_reconnect message",
          );
        }
        break;
      ...
      case "revocation":
        const subscription = (
          payload as { subscription?: { status?: string; type?: string } }
        ).subscription;
        const revocationReason = this.getRevocationReason(subscription?.status);
        console.warn(
          `⚠️ Subscription revoked (${subscription?.type}): ${revocationReason}`,
        );
        break;
  ...
  private async handleClose(event: CloseEvent): Promise<void> {
    ...
    switch (closeCode) {
      ...
      case 4003: // Connection unused
        console.warn(
          "⚠️ Connection unused - reconnecting with fresh connection",
        );
